...
class BookDialog(tk.Toplevel):
    ...
    def on_ok(self):
        ...
        self.result = {"title": title, "author": author, "genre": genre, "year": year,
                       "shelfLocation": shelfLocation, "status": status}
        self.destroy()
...
class WaitingListDialog(tk.Toplevel):
    def __init__(self, parent):
        ...
        # Treeview setup
        self.tree = ttk.Treeview(tree_frame, columns=( "BookID", "MemberID", "Title", "OrderNumber", "DateAdded"),
                                 show="headings")
    ...
    def refresh_waiting_list(self):
        ...
        # Populate the waiting list from the database
    ...
    def delete_entry(self):
        ...
        if selected_item:
            ...
            # Call a method to delete the entry from the waiting list
            DatabaseManager().remove_from_waiting_list(book_id, member_id)
            # Refresh the waiting list to reflect the changes
            self.refresh_waiting_list()
        else:
...
class BookManagementGUI:
    def __init__(self, window):
        ...
        self.tree = ttk.Treeview(tree_frame,
                                 columns=("ID", "Title", "Author", "Genre", "Year", "Shelf Location", "Status"),
                                 show="headings")
        ...
        button_labels = ['Add Book', 'Update Book', 'Delete Book', 'Refresh', 'Search', 'Show Unavailable Books',
                         'Register For Waiting List', 'Show Waiting List']
        actions = [self.add_book, self.update_book, self.delete_book, self.refresh_book_list, self.search_book,
                   self.show_unavailable_books, self.register_waiting_list, self.show_waiting_list]
    ...
    def register_waiting_list(self):
        ...
        # Get the highest order number for the selected book in the waiting list
        highest_order_number = DatabaseManager().get_highest_order_number(book_id)

        # Increment the highest order number by one
        ...
        confirm_button = tk.Button(member_id_window, text="Confirm",
                                   command=functools.partial(self.on_member_id_ok, member_id_entry, member_id_window))

        confirm_button.grid(row=1, column=0, columnspan=2, padx=5, pady=5, sticky='ew')

    def on_member_id_ok(self, member_id_entry, member_id_window):
